postgres:
	docker run --name mypostgres --network bank-network -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go  github.com/vengolan/simplebank/db/sqlc Store

.PHONY: postgres createdb dropdb migrateup migratedown migrateup1 migratedown1 sqlc test start mock
